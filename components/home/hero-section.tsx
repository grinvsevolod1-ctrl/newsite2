...
export function HeroSection() {
  ...
  const { shouldAnimate, shouldUseBlur, mode } = usePerformance()
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 sm:pt-24 pb-8 sm:pb-12">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {mode === "high" ? (
          ...
          <div className="absolute top-1/4 left-1/4 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-primary/15 rounded-full blur-2xl" />
        ) : (
          <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-primary/10 rounded-full blur-xl" />
        )}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-6xl mx-auto text-center space-y-4 sm:space-y-6 md:space-y-8 lg:space-y-12">
          <div
            className={`inline-flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-2.5 md:px-6 md:py-3 rounded-full bg-white/[0.06] ${shouldUseBlur ? "backdrop-blur-xl backdrop-saturate-[200%]" : ""} border border-primary/30 text-[11px] sm:text-xs md:text-sm font-medium transition-all ${shouldAnimate ? "duration-700" : ""} ${
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
          >
            <Sparkles
              className={`h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-primary ${shouldAnimate ? "animate-pulse" : ""} flex-shrink-0`}
            />
          ...
          <h1
            className={`text-3xl sm:text-4xl md:text-6xl lg:text-7xl xl:text-8xl font-bold leading-[1.15] sm:leading-[1.1] md:leading-[1.05] text-balance ${shouldAnimate ? "transition-all duration-1000 delay-200" : ""} px-2 sm:px-4 ${
          ...
          <p
            className={`text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl text-muted-foreground leading-relaxed text-balance max-w-4xl mx-auto ${shouldAnimate ? "transition-all duration-1000 delay-400" : ""} px-2 sm:px-4 ${
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            {t.hero.subtitle}
          ...
          <div
            className={`flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 justify-center items-stretch sm:items-center pt-2 sm:pt-4 md:pt-8 transition-all ${shouldAnimate ? "duration-1000 delay-600" : ""} px-2 sm:px-4 ${
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            <Button
          ...
          <div
            className={`grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 lg:gap-8 pt-8 sm:pt-12 md:pt-16 lg:pt-20 max-w-5xl mx-auto transition-all ${shouldAnimate ? "duration-1000 delay-800" : ""} ${
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            {[
